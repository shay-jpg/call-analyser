...
async function processOneRecording(rec, systemPrompt) {
  ...
  const short = rec.id.substring(0, 8);
  ...
  try {
    ...
    console.log(`[REC ${short}] downloaded in ${Date.now()-t0}ms`);
  } catch (err) {
    console.error(`[REC ${short}] DOWNLOAD FAILED: ${err.message}`);
  ...
  try {
    ...
    console.log(`[REC ${short}] transcribed in ${Date.now()-t0}ms | ${words} words`);
    db.updateRecording(rec.id, { status: 'analyzing', transcript });
  } catch (err) {
    console.error(`[REC ${short}] TRANSCRIPTION FAILED: ${err.message}`);
    db.updateRecording(rec.id, {
      status: 'error',
  ...
  try {
    ...
    console.log(`[REC ${short}] DONE | status:${analysis.lead_status} | model:${ANALYSIS_MODEL} | analyze:${analyzeMs}ms | total:${totalMs}ms | prospect:${analysis.prospect_name || '-'}`);
  } catch (err) {
    console.error(`[REC ${short}] ANALYSIS FAILED: ${err.message}`);
    db.updateRecording(rec.id, {
      status: 'error',
...
async function processJob(jobId) {
  ...
  console.log(`[JOB ${jobId}] Starting — ${job.total_urls} recordings | model:${ANALYSIS_MODEL} | concurrency:${CONCURRENCY}`);
  ...
  console.log(`[JOB ${jobId}] Finished — ${finalJob.completed}/${finalJob.total_urls} | qualified:${finalJob.qualified} | disqualified:${finalJob.disqualified} | callback:${finalJob.callback} | dnc:${finalJob.dnc} | errors:${finalJob.errors}`);
}
...
function cleanupOrphanedTempFiles() {
  ...
  try {
    ...
    for (const f of files) {
      try { fs.unlinkSync(path.join(TEMP_DIR, f)); count++; } catch (_) {}
    }
    if (count > 0) console.log(`[MAINTENANCE] Removed ${count} orphaned temp file(s)`);
  } catch (_) {}
  return count;
}
