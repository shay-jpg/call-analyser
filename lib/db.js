...
function migrate() {
  ...
  try { d.run('ALTER TABLE recordings ADD COLUMN model_used TEXT'); } catch(e) {}
  try { d.run('ALTER TABLE recordings ADD COLUMN processing_ms INTEGER'); } catch(e) {}
...
function getJobStatusBreakdown(jobId) {
  return all(
    "SELECT lead_status, COUNT(*) as count FROM recordings WHERE job_id = ? AND lead_status IS NOT NULL GROUP BY lead_status ORDER BY count DESC",
    [jobId]
  );
}
...
function createPrompt({ id, name, content }) {
...
function purgeOldRecordings(retentionDays) {
  const n = Math.abs(parseInt(retentionDays) || 30);
  const where = `WHERE status IN ('done', 'error')
    AND finished_at IS NOT NULL
    AND finished_at < datetime('now', '-${n} days')
    AND job_id IN (SELECT id FROM jobs WHERE status IN ('completed', 'failed'))`;

  const countRow = get(`SELECT COUNT(*) as cnt FROM recordings ${where}`);
  const toDelete = countRow ? countRow.cnt : 0;
  if (toDelete === 0) return 0;

  run(`DELETE FROM recordings ${where}`);
  save();
  console.log(`[MAINTENANCE] Purged ${toDelete} recording(s) older than ${n} days`);
  return toDelete;
}

// ─── Analytics functions ──────────────────────────────────────

function getAggregateStats(days) {
  const n = days ? Math.abs(parseInt(days)) : null;
  const base = `SELECT
    COALESCE(SUM(total_urls),0) as total_calls,
    COALESCE(SUM(qualified),0) as qualified,
    COALESCE(SUM(disqualified),0) as disqualified,
    COALESCE(SUM(callback),0) as callback,
    COALESCE(SUM(dnc),0) as dnc,
    COALESCE(SUM(errors),0) as errors,
    COUNT(*) as total_jobs
    FROM jobs WHERE status = 'completed'`;

  const currentWhere = n ? ` AND created_at >= datetime('now', '-${n} days')` : '';
  const prevWhere    = n ? ` AND created_at >= datetime('now', '-${n * 2} days') AND created_at < datetime('now', '-${n} days')` : null;

  const empty = { total_calls: 0, qualified: 0, disqualified: 0, callback: 0, dnc: 0, errors: 0, total_jobs: 0 };
  const current = get(base + currentWhere) || empty;
  const prev    = prevWhere ? (get(base + prevWhere) || null) : null;
  return { current, prev };
}

function getRecentFlags(limit) {
  const lim = Math.min(parseInt(limit) || 20, 100);
  return all(
    `SELECT r.id, r.job_id, j.name as job_name, r.url, r.lead_status, r.analysis, r.error, r.finished_at
     FROM recordings r
     JOIN jobs j ON j.id = r.job_id
     WHERE r.lead_status = 'DNC' OR r.status = 'error'
...
module.exports = {
  ...
  createJob, getJob, listJobs, updateJobCounters, setJobStatus, getJobStatusBreakdown,
  getRecording, listRecordingsByJob, updateRecording, getPendingRecordings, getAllRecordingsForExport,
  deleteJob,
  createPrompt, listPrompts, deletePrompt,
  getAggregateStats, getRecentFlags,
  purgeOldRecordings
};
